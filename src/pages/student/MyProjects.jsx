...
export default function MyProjects() {
  const { id } = useParams(); // set when accessed via /student/projects/:id
  ...
  // if URL changes (e.g. navigated from dashboard), sync selectedProject
  useEffect(() => {
                  ...
                  <div className="flex flex-col items-end gap-3 flex-shrink-0">
                    {project.total_price && (
                      <div className="text-right">
                        <p className="text-[11px] text-gray-400 mb-0.5">Project value</p>
                        <p className="text-[15px] font-bold text-gray-900">{Number(project.total_price).toLocaleString()} DZD</p>
                      </div>
                    )}
                    <button
                      onClick={() => setSelectedProject(pid)}
                      className="px-5 py-2 bg-violet-600 hover:bg-violet-700 text-white rounded-xl text-[13px] font-semibold transition-colors"
                    >
                      Open workspace →
                    </button>
                  </div>
...
function Workspace({ projectId, onClose }) {
  ...
  return (
    <div className="px-8 py-10 max-w-7xl">
      <button onClick={onClose} className="flex items-center gap-1.5 text-[13px] text-gray-400 mb-7">
          ...
          <div className="bg-white border border-gray-100 rounded-2xl overflow-hidden">
            ...
            {tasks.length === 0 ? (
              ...
              <table className="w-full text-[13px]">
                ...
                <tbody>
                  {tasks.map((task, i) => (
                    <tr key={task.id} className={i !== tasks.length - 1 ? 'border-b border-gray-50' : ''}>
            ...
            {project.total_price && (
              <div>
                ...
                <p className="text-[14px] font-bold text-gray-900">
                  ...
                  <span className="text-[11px] font-normal text-gray-400 ml-1">DZD</span>
                </p>
              </div>
            )}
            {project.team_size && (
              <div>
                ...
                <p className="text-[13px] font-medium text-gray-700">{project.team_size} members</p>
              </div>
            )}
            <div>
