...
export default function ExpertChat() {
  ...
  // ── Join all conversation rooms on load ───────────────────────────────────
  useEffect(() => {
  ...
  useEffect(() => {
    ...
    const onAnyMessage = (msg) => {
      if (msg.conversation_id === selected?.id) {
        setConversations(prev => bumpConv(prev, msg.conversation_id, msg.content, msg.created_at));
        return;
      }
      setConversations(prev => bumpConv(prev, msg.conversation_id, msg.content, msg.created_at, true));
    };
  ...
  const bumpConv = (prev, convId, content, at, incrementUnread = false) => {
    const updated = prev.map(c =>
      c.id === convId
        ? { ...c, last_message: content, last_message_at: at, unread: incrementUnread ? (Number(c.unread) || 0) + 1 : c.unread }
        : c
    );
  ...
  const handleFileChange = async (e) => {
    ...
    try {
      ...
      const res = await api.post(`/messages/conversations/${selected.id}/file`, formData, {
        headers: { 'Content-Type': 'multipart/form-data' },
      });
      const msg = res.data;
      setMessages(prev => prev.find(m => m.id === msg.id) ? prev : [...prev, msg]);
      setConversations(prev => bumpConv(prev, selected.id, `📎 ${file.name}`, msg.created_at));
    } catch (err) {
          ...
          <div className="px-4 py-4 border-b border-gray-100">
            <div className="relative">
              <input
                ...
                className="w-full pl-9 pr-3 py-2 rounded-lg border border-gray-200 text-sm focus:outline-none focus:ring-2 focus:ring-[#7c3aed] focus:border-transparent"
              />
          ...
          {loadingConvs ? (
            ...
            <button key={c.id} onClick={() => handleSelect(c)}
              className={`w-full text-left px-4 py-3.5 border-b border-gray-50 transition-colors
                ${selected?.id === c.id ? 'bg-[#f5f3ff]' : 'hover:bg-gray-50'}`}
            >
              <div className="flex items-start gap-3">
                <div className="w-10 h-10 rounded-full bg-[#ede9fe] flex items-center justify-center text-xs font-semibold text-[#7c3aed] flex-shrink-0">
                ...
                <div className="flex-1 min-w-0">
                  ...
                  {c.project_title && (
                    <div className="text-[11px] text-[#7c3aed] mb-0.5 truncate">{c.project_title}</div>
                  )}
                  <div className="flex items-center justify-between">
                    <span className="text-xs text-gray-500 truncate">{c.last_message || ''}</span>
                    {Number(c.unread) > 0 && (
                      <span className="w-5 h-5 rounded-full bg-[#7c3aed] text-white text-[10px] font-bold flex items-center justify-center flex-shrink-0 ml-2">
                        {c.unread}
                      </span>
                    )}
                  </div>
            ...
            <>
              ...
              <div className="px-6 py-4 border-b border-gray-200 flex items-center justify-between bg-white">
                <div className="flex items-center gap-3">
                  <div className="w-10 h-10 rounded-full bg-[#ede9fe] flex items-center justify-center text-xs font-semibold text-[#7c3aed]">
                  ...
                  <div>
                    ...
                    {selected.project_title && (
                      <div className="text-xs text-[#7c3aed]">{selected.project_title}</div>
                    )}
                  </div>
                </div>
                ...
                {loadingMsgs ? (
                  ...
                  return (
                    <div key={msg.id} className={`flex ${mine ? 'justify-end' : 'justify-start'} items-end gap-2`}>
                      {!mine && (
                        <div className="w-7 h-7 rounded-full bg-[#ede9fe] flex items-center justify-center text-[10px] font-semibold text-[#7c3aed] flex-shrink-0">
                      ...
                      <div className="flex flex-col max-w-[60%]">
                        ...
                        {/* File message */}
                        {msg.file_url ? (
                          <div className={`rounded-2xl overflow-hidden border ${mine ? 'border-[#7c3aed]' : 'border-gray-200'}`}>
                            {msg.file_type === 'image' ? (
                              <img
                                src={msg.file_url}
                                alt={msg.file_name}
                              ...
                              <a
                                ...
                                className={`flex items-center gap-3 px-4 py-3 ${mine ? 'bg-[#7c3aed] text-white' : 'bg-white text-gray-700'}`}
                              >
              ...
              <div className="px-6 py-4 border-t border-gray-200 bg-white">
                ...
                <div className="flex items-center gap-3">
                  ...
                  <input
                    ...
                    className="flex-1 px-4 py-2.5 rounded-lg border border-gray-200 text-sm focus:outline-none focus:ring-2 focus:ring-[#7c3aed] focus:border-transparent"
                  />
                  <button
                    onClick={handleSend}
                    disabled={!input.trim()}
                    className="px-5 py-2.5 bg-[#7c3aed] text-white rounded-lg text-sm font-medium disabled:opacity-50 disabled:cursor-not-allowed flex items-center gap-2"
                  >
                    <Send size={16} />
                  </button>
                </div>
